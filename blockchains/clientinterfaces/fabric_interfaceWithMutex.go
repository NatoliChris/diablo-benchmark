...
//FabricInterfaceBis is the Hyperledger Fabric implementation of the clientinterface
// Provides functionality to communicate with the Fabric blockchain
type FabricInterfaceBis struct {
	...
	mutex sync.RWMutex					// mutex for TransactionInfo
...
func (f *FabricInterfaceBis) Init(chainConfig *configs.ChainConfig) {
...
func (f *FabricInterfaceBis) populateWallet(wallet *gateway.Wallet, user types.FabricUser) error {
...
func (f *FabricInterfaceBis) Cleanup() results.Results {
...
func (f *FabricInterfaceBis) throughputSeconds() {
...
// metrics
func (f *FabricInterfaceBis) Start() {
...
func (f *FabricInterfaceBis) ParseWorkload(workload workloadgenerators.WorkerThreadWorkload) ([][]interface{}, error) {
...
func (f *FabricInterfaceBis) ConnectOne(id int) error {
	return nil
}

// ConnectAll connects to all nodes given in the hosts
//
func (f *FabricInterfaceBis) ConnectAll(primaryID int) error {
	return nil
}
...
func (f *FabricInterfaceBis) DeploySmartContract(tx interface{}) (interface{}, error) {
	return nil, nil
}

// SendRawTransaction sends the transaction by the gateway
func (f *FabricInterfaceBis) SendRawTransaction(tx interface{}) error {
	...
	if transaction.FunctionType == "write" {
		...
		go func(tr *gateway.Transaction) {
			...
			time := time.Now()
			valid := err == nil
			f.updateThroughput(valid,transaction.ID,time)
		}(t)
...
// updateThroughput updates relevant fields for the benchmark and it ensures that
// concurrent access to TransactionInfo is controlled
func (f *FabricInterfaceBis) updateThroughput(valid bool, id uint64, time time.Time){
	...
	f.TransactionInfo[id] = append(f.TransactionInfo[id], time)
...
func (f *FabricInterfaceBis) SecureRead(callFunc string, callParams []byte) (interface{}, error) {
	return nil, nil
}

// GetBlockByNumber retrieves the block information at the given index
func (f *FabricInterfaceBis) GetBlockByNumber(index uint64) (GenericBlock, error) {
	return GenericBlock{
		Hash:              "",
		Index:             0,
		Timestamp:         0,
...
func (f *FabricInterfaceBis) GetBlockHeight() (uint64, error) {
	return 0, nil
}
...
func (f *FabricInterfaceBis) ParseBlocksForTransactions(startNumber uint64, endNumber uint64) error {
	return nil
}

// Close the connection to the blockchain node
func (f *FabricInterfaceBis) Close() {
	f.Gateway.Close()
}
