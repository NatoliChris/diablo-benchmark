...
// const (
// 	OPCODE_CMD_CREATECLIENT uint8 = 0
// 	OPCODE_CMD_INTERACTION  uint8 = 1
// )


// func encodeSecondaryTags(tags []string) ([]byte, error) {
// 	var buffer bytes.Buffer
// 	var i, n int

// 	if len(tags) > 255 {
// 		return nil, fmt.Errorf("too many tags (%d)", len(tags))
// 	}
...
// 	for i = range tags {
// 		if len(tags[i]) > 255 {
// 			return nil, fmt.Errorf("too large tag '%s'", tags[i])
// 		}
// 		n += len(tags[i])
// 	}
...
// 	buffer.WriteByte(uint8(len(tags)))

// 	for i = range tags {
// 		buffer.WriteByte(uint8(len(tags[i])))
...
// func decodeSecondaryTags(source io.Reader) ([]string, error, int) {
...
// 	_, err = io.ReadFull(source, buffer[:1])
// 	if err != nil {
// 		return nil, err, 0
// 	}
...
// 		if size == 0 {
// 			return nil,fmt.Errorf("corrupted tag size: %d", size),0
// 		}

// 		_, err = io.ReadFull(source, buffer[:size])
// 		if err != nil {
// 			return nil, err, 0
// 		}
...
// 		tags[i] = string(buffer[:size])
// 	}

// 	return tags, nil, done
// }


// func encodeBlockchainSetup(target io.Writer, sysname string) error {
// 	var buf []byte
// 	var err error

// 	if len(sysname) > 255 {
// 		return fmt.Errorf("too large interface name '%s'",
// 			len(sysname))
// 	}

// 	buf = make([]byte, 1)
// 	buf[0] = uint8(len(sysname))
// 	_, err = target.Write(buf)
// 	if err != nil {
// 		return err
// 	}

// 	_, err = io.WriteString(target, sysname)
// 	if err != nil {
// 		return err
// 	}
...
// func decodeBlockchainSetup(source io.Reader) (string, error) {
// 	var buf []byte = make([]byte, 255)
// 	var err error
// 	var n int

// 	_, err = io.ReadFull(source, buf[:1])
// 	if err != nil {
// 		return "", err
// 	}
...
// 	_, err = io.ReadFull(source, buf[:n])
// 	if err != nil {
// 		return "", err
// 	}
...
// func encodeCreateClientCommand(view []string, id int) ([]byte, error) {
// 	var buffer bytes.Buffer
// 	var i, n int

// 	if len(view) > 65535 {
// 		return nil, fmt.Errorf("too many endpoints (%d)", len(view))
// 	}

// 	if (id < 0) || (id > 65535) {
// 		return nil, fmt.Errorf("unsupported client id (%d)", id)
// 	}
...
// 	for i = range view {
// 		if len(view[i]) > 255 {
// 			return nil, fmt.Errorf("too large address '%s'",
// 				view[i])
// 		}
// 		n += len(view[i])
// 	}
...
// 	buffer.WriteByte(OPCODE_CMD_CREATECLIENT)
// 	binary.Write(&buffer, binary.LittleEndian, uint16(len(view)))
// 	binary.Write(&buffer, binary.LittleEndian, uint16(id))

// 	for i = range view {
// 		buffer.WriteByte(uint8(len(view[i])))
// 		buffer.WriteString(view[i])
// 	}

// 	return buffer.Bytes(), nil
// }

// func decodeCreateClientCommand(source io.Reader, handler func([]string, int)) error {
// 	var buf []byte = make([]byte, 255)
// 	var i, n, id, size int
// 	var view []string
// 	var u16 uint16
...
// 	err = binary.Read(source, binary.LittleEndian, &u16)
// 	if err != nil {
// 		return err
// 	}
...
// 		if size == 0 {
// 			return fmt.Errorf("corrupted address size: %d",
// 				size)
// 		}

// 		_, err = io.ReadFull(source, buf[:size])
// 		if err != nil {
// 			return err
// 		}

// 		view[i] = string(buf[:size])
// 	}
...
// func encodeInteractionCommand(time float64, payload []byte) ([]byte, error) {
// 	return nil, nil
// }

// func decodeCommand(source io.Reader, onCreateClient func([]string, int), onInteraction func(float64, []byte)) error {
// 	var opcode []byte = make([]byte, 1)
// 	var err error

// 	_, err = io.ReadFull(source, opcode)
// 	if err != nil {
// 		return err
// 	}

// 	switch (opcode[0]) {
// 	case OPCODE_CMD_CREATECLIENT:
// 		return decodeCreateClientCommand(source, onCreateClient)
// 	default:
// 		return fmt.Errorf("unknown command opcode: %d", opcode[0])
// 	}
// }
