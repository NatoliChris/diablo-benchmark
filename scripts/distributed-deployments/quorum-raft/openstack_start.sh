...
for i in `seq 0 2`; do
  ...
  scp -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null -i $MACHINEKEY -P "812$i" permissioned-nodes.json ubuntu@localhost:~/data
  ...
  ssh -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null -i $MACHINEKEY -p "812$i" -l ubuntu localhost "cp data/permissioned-nodes.json data/static-nodes.json"
