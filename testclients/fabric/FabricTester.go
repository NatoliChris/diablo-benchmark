...
func setupAndRun(createChannel bool, configOpt core.ConfigProvider, test testSDKFunc, sdkOpts ...fabsdk.Option) {
	...
	//ANCHOR check fabsdk object def
...
func createChannelAndCC(sdk *fabsdk.FabricSDK) {
	...
	//ANCHOR Client context creation

	//clientContext allows CREATION OF TRANSACTIOH using the supplied identity as the credential.
	clientContext := sdk.Context(fabsdk.WithUser(orgAdmin), fabsdk.WithOrg(ordererOrgName))

	//ANCHOR Resource managment client
...
func createCC(orgResMgmt *resmgmt.Client) {
	...
	//ANCHOR Installing cc on org peers
...
func initCC(sdk *fabsdk.FabricSDK) {
	...
	_, err = client.Execute(channel.Request{ChaincodeID: ccID, Fcn: "init", Args: integration.ExampleCCInitArgsLc(), IsInit: true},
		channel.WithRetry(retry.DefaultChannelOpts))
	if err != nil {
		fmt.Println("Failed to init: %s", err)
	}
